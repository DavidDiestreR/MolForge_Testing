...
## 🧠 Uso de Jupyter con múltiples entornos (Windows, WSL y Ubuntu)

Este proyecto utiliza **dos entornos Conda**:
| Entorno | Uso | Kernel recomendado |
|----------|-----|--------------------|
| `MolForge_env` | Modelo MolForge (CPU) | `Python (MolForge_env)` |
| `molforge-tools` | Preprocesado RDKit + pandas | `Python (molforge-tools)` |

Para poder usarlos en **Jupyter o VSCode**, debes **registrarlos como kernels** con `ipykernel`.
El comando cambia ligeramente según **dónde esté el entorno** y **desde dónde trabajes**.

### ✅ Caso 1 – Todo en Windows (sin WSL)
Ejecutar en **Anaconda Prompt** de Windows:
```bash
conda activate MolForge_env
...
### 🐧 Caso 2 – Todo en Linux (Ubuntu en laboratorio)
Ejecutar en terminal Linux normal:
```bash
conda activate MolForge_env
python -m ipykernel install --user --name MolForge_env --display-name "Python (MolForge_env)"
conda activate molforge-tools
...
### 💻 Caso 3 – Proyecto en Windows pero entorno en WSL (recomendado)
Cuando el código está en `/mnt/d/...` pero conda está instalado en **WSL**.

Registrar kernels **desde WSL**:
```bash
conda activate MolForge_env
python -m ipykernel install --user --name MolForge_env --display-name "Python (MolForge_env WSL)"
conda activate molforge-tools
python -m ipykernel install --user --name molforge-tools --display-name "Python (molforge-tools WSL)"
```

Luego lanzar Jupyter desde WSL:
```bash
jupyter lab --no-browser --ip=0.0.0.0
```

Y abrirlo desde el navegador en Windows usando el enlace con token que muestra.
